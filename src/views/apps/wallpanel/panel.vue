...
<VCard class="container-fluid">
   ...
   <VCardText>
       ...
       <VForm ref="refForm" v-model="isFormValid">
         <VRow @click="getdata(rowid);openFullscreen('left_grid'+rowid)">
           <!-- ?? Email -->
          
             
              <VCol cols="9"  >
                <div v-html="divdata" style=" height:100px;">
       ...
       <VBtn @click="compact(0)" class="btn btn-primary" href="#">Compact</VBtn -->
         <div class="board" :class="{ 'hidden': !isvisible }">
 
            <div class="grid-stack" :id="'left_grid'+rowid" style="display:none">
              <VBtn @click="closeFullscreen('left_grid'+rowid);">
                Exit Fullscreen</VBtn> 
...
export default {
  ...
  mounted() {
    ...
    this.divdata='<div style="background-color:#cdcdcd;width:70px;height:50px; float:left"></div>'       
  ...
  methods:{
    ...
    getdata(id){
      axios.post(URL()+`/wallpanel_template`,{
          ...
          .then((response) => {
            this.currentChildComponent='Attendance'
