...
if (deviceData.status == "success") {
    ...
    console.log(deviceData);
    // refetch role
    message.value = deviceData.message;
...
if (deviceData.status == "failed") {
    ...
    err.value = deviceData.message;
    console.log("Refech leave type failed");
}
 
}

const updatedevice= (deviceData,index) => {
 //tags.value.splice(index, 1,deviceData);
 console.log('udpate device')
 console.log(deviceData)
 i=0
  if (deviceData.status == "success") {
    ...
    axios
      ...
      .then((response) => {
        //console.log(response.data.data);
                    isErrorAlertVisible.value = false;
                    isAlertVisible.value = true;
                    message.value = "Device Deleted ";
                    currentPage.value=1
                    fetchTags(0)       //   this.listings = response.data; 
      })
      .catch((error) => {
          console.log(error)
          isErrorAlertVisible.value = true;
...
<template>
<section v-if="$can('read', 'PID_DEVICE')">
     <!-- ?? Alert messages   -->
    ...
    <VRow>      
      ...
      <VCard >
          ...
          <VCardText class="d-flex flex-wrap py-4 gap-4">
           ...
           <div>
             <h3 class="title-header">{{$t('Device Setup')}}</h3>
           </div>
           <VSpacer />
            ...
            <tr
              ...
              >
                ...
                <VBtn
                    ...
                    color="default"
                    variant="text"
                    v-if="$can('read', 'PID_DEVICE')"
                    @click="(isViewDeviceDrawerVisible = true),(getDataDevice=tag),(getIndex=index)"
                  >
                    <VIcon
                      size="22"
                      icon="tabler-eye"
                  ...
                  <VBtn
                    ...
                    color="default"
                    variant="text"
                    v-if="$can('update', 'PID_DEVICE')"
                    @click="(isEditDeviceDrawerVisible = true),(getDataDevice=tag),(getIndex=index)"
                  >
                    <VIcon
                      size="22"
                      icon="tabler-edit"
    ...
    <EditDeviceDrawer
      v-model:isDrawerOpen="isEditDeviceDrawerVisible"
       v-bind:datadevice="getDataDevice"
