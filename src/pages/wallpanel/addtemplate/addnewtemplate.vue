...
import axios from "axios"
    ...
    import { widgetypeList } from "@/helpers/helper"
    import { 
      emailValidator, 
      requiredValidator,
      lengthValidator,
...
const widgetList = () => {
    ...
    .then((response) => {
        ...
        widget_list.value=data;

        console.log(widget_list);
    })   
   
}
watchEffect(widgetList)
...
<VCard class="container-fluid">
   ...
   <VCardText>
   <h3 class="title-header">Template - Create</h3>
   </VCardText>
   <VSpacer />
   <VCardText>
       <!-- ?? Form -->
       <VForm ref="refForm" v-model="isFormValid" @submit.prevent="onSubmit">
         <VRow>
           <!-- ?? Email -->
           <VCol cols="4">
            <VTextField v-model="template_name" :rules="[requiredValidator]" label="Template Name" />
              </VCol>
             
              <VCol cols="4"  class="think">
              <VTextField v-model="template_description" :rules="[requiredValidator]" label="Template Description" />
              </VCol>  
              <VCol cols="4"  class="think">
                <VBtn @click="openFullscreen('left_grid');" type="button"  class="btn btn-primary">Open Fullscreen</VBtn>
          
              </VCol>   

           <VCol cols="3">
            <div id="trash" style="padding: 15px; margin-bottom: 15px;" class="bg-primary">
            ...
            <div class="trash text-center text-white">
              ...
              <!-- manually force a drop size of 2x1 -->
                  ...
                  <div class="grid-stack-item mt-2 " style="border-bottom:1px solid #cdcdcd" gs-w="2" gs-h="1" gs-max-w="3" v-for="(data,index) in widget_list" >
                    <div class="grid-stack-item-content">{{data.widget_type.name}}</div>
                  </div>
             
             
            </div>
              <div class="sidebar bg-primary">
                <div class="text-center card  grid-stack-item " v-for="wiget in widget_list">
                 
                    <div class="card-body grid-stack-item-content drag mt-2" style="border-bottom:1px solid #cdcdcd" >
                    <span class="grid-stack-item-content"> {{wiget.name}}</span>            
                        
                    
                    </div >
                  
                        <div class="grid-stack-item mt-2" gs-w="2" gs-h="1" gs-max-w="3" v-for="wiget_data in wiget.widget_type">
                          <div class="grid-stack-item-content">{{wiget_data.name}}</div>
                        </div>
              ...
              <a class="btn btn-primary" @click="save()" href="#">Save layout</a>
        ...
        <div class="board">
         ...
         <VRow>
          ...
          <VCol cols="10">
            ...
            <VBtn class="me-3" @click="confirmlayout">
                            Confirm Layout
                        </VBtn>
                        <VBtn type="submit"  class="me-3"   :disabled="isDisabledsubmit">
                            Create
                        </VBtn>
    ...
    <VCard :title="item_title">
      <VCardText>
        <VRow>
          ...
          >
          <VTextField v-model="itemID" :rules="[requiredValidator]" type="hidden" />
            
          <VSelect v-model="widget_type" label=" Widget Type" :rules="[requiredValidator]" :items="widgetype_list"
          @change="selectwidgetype"
          />
          <input id="selectwidgetypeValue" value="" />
            
          </VCol>
          <VCol cols="12">
            <VSelect v-model="widget_name" label=" Widget Name" :rules="[requiredValidator]" :items="widget_list" />
...
export default {
  ...
  mounted() {
   ...
   watchEffect(GridStack.setupDragIn('.sidebar .grid-stack-item', { appendTo: 'body', helper: myClone }));
  ...
  methods:{
   ...
   console.log(list1)
    ...
    console.log(list2)
      ...
      console.log(widget)
      var items=[]
      this.wallpanel_components=[]
      options1.children.forEach( item => {
       console.log(item)
      if(item.id ===  this.itemID){

        item.content=' '+item.id+' '+'<p>'+widgetype+'</p><p>'+widget+'</p>'
      }
       
       items.push(item) 
       this.wallpanel_components.push({
        component_seq_number:item.id,
        width:item.w,
        height:item.h,
        component_x:item.x,
        component_y:item.y,
