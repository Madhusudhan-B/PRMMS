...
const fetchWorkshop = async ( offset, search) => {
  try {
    ...
    const response = await axios.post(
      URL() + `/reports/people_currently_present`,
      {
        requestType: "view",
        offset: offset ,
...
const prepareDataForExport = () => {
  return tags.value.map(user => {
    return {
      ...
      'IN/Out State': user.in_out_state
...
const downloadXLSX = () => {
  ...
  const fileName = 'Currently-Present-Report.xlsx';
...
<!-- </VForm> -->
          ...
          <VCol cols="6" sm="4">
            ...
            <VCol cols="6" sm="6"> 
          <h3 class="" style="color:#2f2774; margin-top:15px">Currently Present Report</h3>
          </VCol>
        </VRow>
        <div class="search-params">{{ searchParamsString }}</div>
          ...
          <VTable>
            ...
            <tr
              ...
              >
                ...
                <td>
                <span>{{ moment(user.access_timestamp, 'YYYY-MM-DDTHH:mm:ssZ').format('DD-MMM-YY HH:mm') }}</span>
          ...
          </VTable><div class="hidden-on-export">
            <VRow>
          <VCol cols="6" sm="4">
