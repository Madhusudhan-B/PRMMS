...
search.value=searchParams
  ...
  const startDate = moment(startDateString, 'YYYY-MM-DDYYYY-MM-DDTHH:mm:ss').format('YYYY-MM-DDTHH:mm:ss');

  if (endDateString) {
    const endDate = moment(endDateString, 'YYYY-MM-DDYYYY-MM-DDTHH:mm:ss').format('YYYY-MM-DDTHH:mm:ss');
...
<template>
<section v-if="$can('read', 'OR_AUDIT_REPORT')">
     <!-- ?? Alert messages   -->
          ...
          <VCardText>
            ...
            <VCol
              ...
              <AppDateTimePicker
                    ...
                    :config="{ mode: 'range', timePicker: true }"
                    density="compact"
                  />
