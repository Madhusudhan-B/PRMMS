<script setup>
    import AddNewLocationDrawer from "@/views/apps/settings/list/AddNewLocationDrawer.vue";

    import EditLocationDrawer from "@/views/apps/settings/list/EditLocationDrawer.vue";

    import ViewLocationDrawer from "@/views/apps/settings/list/ViewLocationDrawer.vue";
    ...
    return tags.value.filter((tag) => {
        if (tag) {
            ...
            const locationShortCode = tag.location_short_code ? tag.location_short_code.toLowerCase() : '';
            const locationDescription = tag.location_description ? tag.location_description.toLowerCase() : '';
            const locationType = tag.location_type ? tag.location_type.toLowerCase() : '';
         
           
            return (
                locationShortCode.includes(query) ||
...
fetchTags();
    ...
    const isAddNewLocationDrawerVisible = ref(false);
    const isEditLocationDrawerVisible = ref(false);
    const isViewLocationDrawerVisible = ref(false);
    const getDataLocation = ref([]);
    const getIndex = ref("");
    ...
    const addLocation = (locationData) => {
        ...
        if (locationData.status == "success") {
            ...
            // refetch role
            message.value = locationData.message;
            console.log("Refech Role");
            console.log(tags);
        }

        if (locationData.status == "failed") {
            isErrorAlertVisible.value = true;
            isAlertVisible.value = false;
            err.value = locationData.message;
            console.log("Refech Role failed");
        }
    };

    const updateLocation = (locationData, index) => {
...
<template>
    <section v-if="$can('read', 'SETTINGS_LOCATION')">
        <!-- ?? Alert messages   -->
        ...
        <VRow>
           ...
           <div>
             <h3 class="title-header">{{$t('Location')}}</h3>
           </div>
           <VSpacer />
           ...
           <VSelect
            ...
            <!-- ?? Add user button -->
            <VBtn prepend-icon="tabler-plus" v-if="$can('create', 'SETTINGS_LOCATION')" @click="isAddNewLocationDrawerVisible = true">
                                {{$t('Add Location')}}
                            </VBtn>
           </div>
         </VCardText>	
                    ...
                    <VTable class="text-no-wrap">
                        ...
                        <!-- ?? table body -->
                        <tbody>
                            <tr v-for="(tag,index) in filteredTags" :key="tag.id" style="height: 3.75rem;">
                                <!-- ?? User -->
                                <td>
                                ...
                                <td>
                                    ...
                                    <span > {{tag.location_short_code}}</span>
                                </td>
                                <td>
                                    <span >{{ tag.location_description }}</span>
                                </td>
                                ...
                                <td class="text-center" style="width: 5rem;">
                                    <VBtn icon size="x-small" color="default" variant="text" v-if="$can('read', 'SETTINGS_LOCATION')" @click="(isViewLocationDrawerVisible = true),(getDataLocation=tag),(getIndex=index)">
                                        <VIcon size="22" icon="tabler-eye" />
                                    </VBtn>

                                    <VBtn icon size="x-small" color="default" variant="text" v-if="$can('update', 'SETTINGS_LOCATION')" @click="(isEditLocationDrawerVisible = true),(getDataLocation=tag),(getIndex=index)">
                                        <VIcon size="22" icon="tabler-edit" />
                                    </VBtn>

                                    <VBtn icon size="x-small" color="default" variant="text">
                                        <VIcon size="22" icon="tabler-trash" v-if="$can('delete', 'SETTINGS_LOCATION')" @click="deleteEvent(tag.id,index)" />
                                    </VBtn>
                                </td>
                            </tr>
                        </tbody>
        ...
        <!-- ?? Add New rfidpeople -->
        <AddNewLocationDrawer v-model:isDrawerOpen="isAddNewLocationDrawerVisible" @location-data="addLocation" />
        <!-- ?? Update  rfidpeople -->
        <EditLocationDrawer v-model:isDrawerOpen="isEditLocationDrawerVisible" v-bind:datalocation="getDataLocation" @location-data="updateLocation" @location-index="getIndex" />
        <!-- ?? View  rfidpeople -->
        <ViewLocationDrawer v-model:isDrawerOpen="isViewLocationDrawerVisible" v-bind:datalocation="getDataLocation" />
    </section>
</template>
